...
AcquisitionKinect2::AcquisitionKinect2():
    ...
    {
        m_bValid = false;
        cout << "[GUIDO] Kinect 2.0 not found\n" << endl;
        return;
    }
    else
        cout << "[GUIDO] Kinect 2.0 found\n" << endl;


    if (m_pKinectSensor)
    {
        cout << "[GUIDO] Opening Kinect 2.0...\n" << endl;
    ...
    if (bAvailable)
        cout << "[GUIDO] Kinect available \n" << endl;
    else
        cout << "[GUIDO] Kinect not available \n" << endl;

    if (!m_pKinectSensor || FAILED(hr))
    {

        cout << "[GUIDO] Error opening Kinect 2.0 \n" << endl;
        m_bValid = false;
        return;
    }
...
AcquisitionKinect2::~AcquisitionKinect2(){
    ...
    SafeReleaseB(m_pColorFrameReader);
    SafeReleaseB(m_pDepthFrameReader);
    SafeReleaseB(m_pBodyFrameReader);


    // close the Kinect Sensor
    if (m_pKinectSensor)
    {
        printf("[GUIDO] closing Kinect 2.0\n");
        m_pKinectSensor->Close();
    }

    SafeReleaseB(m_pKinectSensor);
}
...
void AcquisitionKinect2::InitColorReader(){
    ...
    if (!SUCCEEDED(hr))
        printf("[GUIDO] Error opening color frame source Kinect 2.0 \n");
    else
        printf("[GUIDO] Color frame source opened! \n");

    SafeReleaseB(pColorFrameSource);
}
...
void AcquisitionKinect2::InitDepthReader(){
    ...
    if (!SUCCEEDED(hr))
        printf("[GUIDO] Error opening depth frame source Kinect 2.0 \n");
    else
        printf("[GUIDO] Depth frame source opened! \n");

    SafeReleaseB(pDepthFrameSource);
}
...
void AcquisitionKinect2::InitBodyReader()
{
    ...
    if (!SUCCEEDED(hr))
        printf("[GUIDO] Error opening body frame source Kinect 2.0 \n");
    else
        printf("[GUIDO] Body frame source opened! \n");

    SafeReleaseB(pBodyFrameSource);
...
void AcquisitionKinect2::InitIndexBodyReader(){
    ...
    if (!SUCCEEDED(hr))
        printf("[GUIDO] Error opening body index frame source Kinect 2.0 \n");
    else
        printf("[GUIDO] Body index frame source opened! \n");

    SafeReleaseB(pBodyIndexFrameSource);
}
...
//        SafeReleaseB(m_pFaceFrameSources[i]);
//    }
//
//}
...
void  AcquisitionKinect2::CheckKinect(){
    ...
    if (m_pKinectSensor)
    {
        ...
        if (!bris)
        {	printf("[GUIDO] Kinect close. Opening \n");
            m_pKinectSensor->Open();

            m_pKinectSensor->get_IsAvailable(&bris);
            if (!bris)
            {
                printf("[GUIDO] Kinect not available \n");
...
void AcquisitionKinect2::getFrame(FrameSet &frame){
    ...
    if (pColorFrameReader)
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            SafeReleaseB(pFrameDescription);
        }
        SafeReleaseB(pColorFrame);
    }
            ...
            if (SUCCEEDED(hr))
            {
                ...
                for (y=0; y<nHeight; y++)
                    for (x=0; x<nWidth ;x++){
						...
						risdef2(y, x) = (ris2.at<UINT16>(y, x) - 500) / 8;
                    }

                frame.depth = risDef.clone();
                frame.depthWithoutSkeleton = risDef.clone();
				frame.depthWithoutSkeleton1ch = risdef2.clone();
            }
            SafeReleaseB(pFrameDescription);
        }
        SafeReleaseB(pDepthFrame);
    }
